...
import { HelpCircle, Send, Upload, X, FileText, Loader, Clock } from 'lucide-react';
...
export default function HomeworkPage() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      // Refresh history
      homeworkApi.history().then(r => setHistory(r.data.history || [])).catch(() => {});
    } catch (err: any) {
  ...
  const loadFromHistory = (hw: any) => {
    setMessages([
      { role: 'user', content: hw.question },
      { role: 'assistant', content: hw.answer },
    ]);
  ...
  return (
    <AppLayout>
      <div className="max-w-4xl mx-auto flex gap-6 h-[calc(100vh-4rem)]">

        {/* History sidebar */}
        <AnimatePresence>
          {showHistory && (
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
              className="w-64 flex-shrink-0 flex flex-col gap-2 overflow-y-auto">
              <div className="flex items-center justify-between mb-1">
                <h2 className="font-bold text-sm text-slate-400 uppercase tracking-widest">History</h2>
                <button onClick={() => setShowHistory(false)} className="text-slate-500 hover:text-white">
                  <X className="w-4 h-4" />
                </button>
              </div>
              {history.length === 0 ? (
                <div className="text-sm text-slate-500 text-center py-8">No history yet</div>
              ) : (
                history.map((hw: any) => (
                  <button key={hw.id} onClick={() => loadFromHistory(hw)}
                    className="card !py-3 !px-3 text-left hover:border-brand-500/30 transition-all group">
                    <div className="text-xs font-medium text-slate-300 truncate group-hover:text-white">
                      {hw.question}
                    </div>
                    {hw.subject && (
                      <div className="text-xs text-slate-600 mt-1">{hw.subject}</div>
                    )}
                  </button>
                ))
              )}
        ...
        <div className="flex-1 flex flex-col min-w-0">
          {/* Header */}
          <div className="flex-shrink-0 pb-4 border-b border-surface-border mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-slate-400 mt-1 text-sm">Ask any question or upload a PDF.</p>
            </div>
            <button onClick={() => setShowHistory(!showHistory)}
              className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-sm font-medium transition-all ${showHistory ? 'bg-brand-500/20 border-brand-500/40 text-brand-400' : 'border-surface-border text-slate-400 hover:text-white'}`}>
              <Clock className="w-4 h-4" />
              History {history.length > 0 && <span className="text-xs bg-brand-500/20 text-brand-400 px-1.5 py-0.5 rounded-full">{history.length}</span>}
            </button>
          </div>
